# U-Nav Web Application

A comprehensive campus navigation system built with React, TypeScript, and Vite, featuring immersive 3D visualization and intelligent pathfinding.
...
- **Interactive 3D & 2D Maps**: Navigate campus buildings with detailed floor plans and room indicators.
- **Intelligent Pathfinding**: Smart routing between buildings using a simple pathfinding algorithm that accounts for obstacles.
- **Smart Tracking**: Real-time GPS synchronization with a high-accuracy 3D world representation.
- **Contextual View Modes**: Seamless switching between 3D perspective and 2D-like top-down modes.
- **Camera Persistence**: Auto-save and restore camera position between sessions.
- **World Boundaries**: Restricted navigation area to keep the user focused on the campus., hours, and location data.
- **About Section**: Detailed information about the university and the navigation system.
- **User Authentication**: Secure login and signup system with university email validation.
- **Guest Mode**: Limited view-only access for quick navigation without an account.
...
- **Scene**: Campus scene loaded from high-fidelity GLB models (`public/models/`).
- **Pathfinding**: Custom simple pathfinding algorithm that calculates the shortest path between any two points while avoiding building hitboxes and obstacles.
- **Camera**: Intelligent `MapControls` integration with automatic centering on the user's `geolocation` and rigid world boundaries.
- **Interactivity**: Clickable building zones with HTML-based labels and floor-by-floor detail views.
